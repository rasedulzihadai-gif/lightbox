...
## THE 7 RULES THAT MATTER MOST — read these first, they are checked last
These seven rules are graded automatically after you answer. Models that skip
...
G. TITLE LENGTH (hard limit, per platform — count characters before
   finalizing): Adobe Stock and Freepik/Vecteezy titles must be 70
   characters or fewer — this is a ranking best-practice, not just a hard
   cap, so treat 70 as the real ceiling, not the 200/100-character hard
   max those platforms merely tolerate. Shutterstock titles must be
   50-100 characters. iStock/Getty titles should stay concise — under 70
   characters unless the extra length is truly necessary to stay literal
   (rule E). If a literal, rule-E-compliant title runs long, tighten
   wording (shorter synonyms, drop a redundant word) rather than deleting
   the color or orientation info rule E requires — never solve a
   length problem by becoming vaguer.
...
each title for a literal-over-subjective description AND its character
count against rule G, and scan every tag for rule-F violations. Fix any
list that fails, then output.
...
### Adobe Stock
- Title: hard max 200 characters, but rule G's real ceiling is 70 — natural sentence, NO commas.
- Keywords: 25-40 keywords, hard max 49, hard min 5. First 10 weighted heaviest.

### Shutterstock
- Title: 50-100 characters (rule G), readable descriptive sentence.
- Keywords: 30-40 keywords (max 50, min 7). First 7 weighted heaviest. Include broad AND specific synonyms.

### iStock / Getty Images
- Title: precise, factual, no embellishment, concise per rule G.
...
- Title: short, clear, natural phrase — rule G's 70-character ceiling applies (hard max 100). No file-type mentions.
...
- [ ] Title character count fits this platform's rule-G limit (70 for Adobe/Freepik/iStock, 50-100 for Shutterstock)?
...
Reminder before you answer — check each platform's keyword list against these 7 counts:
...
7) title character count: 70 or under for Adobe/Freepik/iStock, 50-100 for Shutterstock — count it, don't estimate it, and tighten wording rather than dropping required color/orientation info if it runs long
