...
// Recovery escape hatch: remove EVERY localStorage key this app owns
// (mstock_*). Used by the error boundaries and the Settings footer, so a
// user whose saved data has gone bad can self-recover in one click —
// without opening DevTools. Keys are collected first, then removed, so the
// live key(0..length) scan never skips an entry.
